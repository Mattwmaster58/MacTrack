...
def create_all_respecting_table_flags(bind: AsyncDbEngine) -> None:
    ...
    tables = [
        x for x in Base.metadata.tables.values() if not getattr(x, EXCLUDE_FROM_CREATION_KEY, False)
    ]
    return Base.metadata.create_all(bind, tables)


async def init_db_from_app(_app: Litestar) -> None:
    return await init_db_from_engine(
        getattr(_app.state, SQLAlchemyAsyncConfig.engine_app_state_key)
    )
